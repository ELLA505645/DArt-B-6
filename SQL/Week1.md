...
SQL 쿼리문은 아래와 같은 문법으로 작성한다.
SELECT, FROM, WHERE

SELECT: 조회할 열(컬럼) 선택
FROM: 데이터를 가져올 테이블 지정
WHERE: 조건에 맞는 행(레코드) 필터링
...
<!-- 새롭게 배운 내용을 자유롭게 정리해주세요.-->
집계: 여러 행을 모아서 하나의 요약값으로 만드는 것
그룹화: 특정 기준으로 데이터를 묶어서 집계하는 것

GROUP BY:데이터를 그룹으로 묶음
HAVING: 그룹화하 결과에 조건 걸 때 사용
ORDER BY: 결과를 정렬 (오름차순/내림차순)
집계함수: SUM(합계),COUNT(개수),AVG(평균),MAX(최댓값),MIN(최솟값)

HAVING: 그룹화된 결과에 조건, 집계 결과에 필터링
WHERE: 개별 행에 조건, 그룹화하기 전에 필터링

# 2️⃣ 학습 인증란

![학습 인증](./다트비week1인증샷.png)
